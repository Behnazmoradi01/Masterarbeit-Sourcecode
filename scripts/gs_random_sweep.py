...
from gs_division.basic import goldschmidt_classic_divide, threshold_for_precision
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description=(
            "Random sweep for Goldschmidt division convergence statistics. "
    ...
    ap.add_argument("--out", type=str, default="gs_random_sweep_summary.csv")
    args = ap.parse_args()
    if args.n <= 0:
    ...
    for precision in ["fp8", "bf16", "fp32"]:
        ...
        failures = 0
        for b in bs:
            _, iters, errs = goldschmidt_classic_divide(
                args.a,
                b,
                precision=precision,
