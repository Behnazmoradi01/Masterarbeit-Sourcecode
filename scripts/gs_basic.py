...
from gs_division.basic import PRECISION_THRESHOLDS, goldschmidt_classic_divide, threshold_for_precision
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description=(
            "Goldschmidt division q=a/b with hardware-like normalization (frexp/ldexp) and "
            "classic iteration a<-a*(2-b), b<-b*(2-b)."
        )
    )
    ap.add_argument("--a", type=float, required=True, help="Dividend a (float)")
    ...
    q, iters, errors = goldschmidt_classic_divide(
        args.a,
        args.b,
        threshold=threshold,
