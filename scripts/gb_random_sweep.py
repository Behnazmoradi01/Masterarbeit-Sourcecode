...
from gs_binomial.basic import goldschmidt_binomial_divide, threshold_for_precision
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description=(
            "Random sweep for Goldschmidt-binomial division convergence statistics. "
            "Samples b log-uniformly over [2**exp_min, 2**exp_max]."
        )
    )
    ap.add_argument("--a", type=float, default=1.0, help="Dividend a (constant for the sweep)")
    ap.add_argument("--n", type=int, default=100_000, help="Number of random b samples")
    ap.add_argument("--exp-min", type=float, default=-20.0)
    ap.add_argument("--exp-max", type=float, default=20.0)
    ap.add_argument("--max-iter", type=int, default=50)
    ap.add_argument("--order", type=int, choices=[2, 3], default=3)
    ap.add_argument("--seed", type=int, default=0, help="Random seed (0 uses system entropy)")
    ap.add_argument("--out", type=str, default="gb_random_sweep_summary.csv")
    args = ap.parse_args()
    if args.n <= 0:
    ...
    for precision in ["fp8", "bf16", "fp32"]:
        ...
        failures = 0
        for b in bs:
            _, iters, errs = goldschmidt_binomial_divide(
                args.a,
                b,
                precision=precision,
